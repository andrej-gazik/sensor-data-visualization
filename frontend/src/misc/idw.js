	...
	this.calculateMatrix = function () {
		...
		function calcVal(v, points, p) {
			...
			for (var i = 0; i < points.length; i++) {
				const dist = calcDist(v, points[i]);
				top += points[i].val / Math.pow(dist, p);
				bot += 1 / Math.pow(dist, p);
			}
			if (bot == 0) {
				//When the distance between point and sensor is 0 (the point IS the sensor) Gives black spots, so fix this
				return top;
			} else {
				return top / bot;
			}
